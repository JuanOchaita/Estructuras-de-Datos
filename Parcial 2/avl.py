...
# 2.1 Crear Archivo AVL Tree

"""PROMT:

quiero que segun hagas un AVL segun el siguiente codigo:
...
class AVLTree:
    ...
    def _insert(self, value, node):
        ...
        # Caso izquierdo-izquierdo (rotación simple derecha)
        if balance > 1 and self.get_balance(node.left_child) >= 0:
            return self.rotate_right(node)
            
        # Caso izquierdo-derecho (rotación doble izquierda-derecha)
        if balance > 1 and self.get_balance(node.left_child) < 0:
            node.left_child = self.rotate_left(node.left_child)
            return self.rotate_right(node)
            
        # Caso derecho-derecho (rotación simple izquierda)
        if balance < -1 and self.get_balance(node.right_child) <= 0:
            return self.rotate_left(node)
            
        # Caso derecho-izquierdo (rotación doble derecha-izquierda)
        if balance < -1 and self.get_balance(node.right_child) > 0:
    ...
    def balance(self, node):
        ...
        if balance > 1:
            if self.get_balance(node.left_child) >= 0:  # Caso izquierda-izquierda
                return self.rotate_right(node)
        ...
        # Caso derecho pesado
        if balance < -1:
            if self.get_balance(node.right_child) <= 0:  # Caso derecha-derecha
                return self.rotate_left(node)
    ...
    def _search(self, value, node):
        ...
        if value == node.data:
            ...
            return self._search(value, node.left_child)  # Search left subtree
        
        return self._search(value, node.right_child)  # Search right subtree
